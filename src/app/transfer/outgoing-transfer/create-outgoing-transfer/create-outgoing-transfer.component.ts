...
export class CreateOutgoingTransferComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (
      ...
      history.state.name != undefined &&
      history.state.name == "edit-outgoing-transfer"
    ) {
  ...
  initialNumber() {
    this._outgoingTransferAppService.getLastNumber().subscribe((result) => {
