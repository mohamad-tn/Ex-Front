...
export class EditTreasuryActionComponent
  ...
  implements OnInit
{
      ...
      this._treasuryActionAppService
        ...
        .subscribe((result) => {
          ...
          if (this.treasuryAction.exchangePartyClientId != undefined) {
            ...
            selectedExchangeParty = this.exchangeParties.find(
              (x) => x.exchangePartyId == value
            );
  ...
  initialCompanies() {
    this._companyAppService.getAll().subscribe((result) => {
  ...
  initialBeneficaries(currencyId) {
    ...
    if (currencyId != undefined) {
      this._incomeTransferDetailAppService
        .getNotReceived(currencyId)
        .subscribe((result) => {
  ...
  onActionTypeChange(args: any) {
    ...
    if (this.treasuryAction.currencyId != undefined) {
      if (
        ...
        this.treasuryAction.mainAccount == 0
      ) {
        this.getBalanceByClient(
          this.treasuryAction.mainAccountClientId,
          this.treasuryAction.currencyId
        );
      } else if (
        this.treasuryAction.mainAccountCompanyId != undefined &&
        this.treasuryAction.mainAccount == 1
      ) {
        this.getBalanceByClient(
          this.treasuryAction.mainAccountCompanyId,
          this.treasuryAction.currencyId
        );
      }
    }
  ...
  onExchangePartyChange(args: any) {
    if (args.itemData != undefined) {
      ...
      if (args.itemData.group == "Client") {
        ...
        this.getExchangePartyBalanceByClient(
          args.itemData.id,
          this.treasuryAction.currencyId
        );
        ...
        this.getExchangePartyBalanceByCompany(
          args.itemData.id,
          this.treasuryAction.currencyId
        );
        this.showExchangePartyBalance = true;
  ...
  onBeneficiaryChange(args: any) {
    if (args.itemData.id != undefined) {
      let incomeTransferDetail = this.incomeTransferDetails.find(
        (x) => x.id == args.itemData.id
      );
    ...
    this._clientAppService
      ...
      .subscribe((result) => {
        ...
        if (this.treasuryAction.actionType == 0) {
          //صرف
          this.currentBalance =
            this.treasuryAction.amount != undefined
              ? result.balance + this.treasuryAction.amount
              : result.balance;
        } else {
          //قبض
          this.currentBalance =
  ...
  getBalanceWithCurrency(number) {
    if (this.treasuryAction.currencyId != undefined) {
      let currency = this.currencies.find(
        (x) => x.id == this.treasuryAction.currencyId
      );
  ...
  // print() {
  //   this._modalService
  //     .open(PrintTreasuryActionComponent, {
  //       context: {
  ...
  name = "Treasury-Action-Statement";
  ...
  getExpenseName(): string {
    if (this.treasuryAction.expenseId == undefined) return "";

    return this.expenses.find((x) => x.id == this.treasuryAction.expenseId)
      ?.name;
  }

  getIncomeName(): string {
