  ...
  implements OnInit
{
  ...
  initialNumber() {
    this._treasuryActionAppService.getLastNumber().subscribe((result) => {
  ...
  initialExchangeParties() {
    this._treasuryActionAppService.getExchangeParties().subscribe((result) => {
      ...
      this.exchangeParty.exchangePartyId =
        this.exchangeParties[0].exchangePartyId;
    });
  }
  ...
  searchSearchTreasuryDialog() {
    this._modalService
      .open(SearchTreasuryActionDialogComponent)
      .onClose.subscribe((e: any) => {
        if (e) {
